...
class DashboardManager {
  ...
  updateStatsCards(stats) {
    ...
    if (salesElement) {
      salesElement.textContent = `$${stats.totalSales.toLocaleString("es-ES", {
    ...
    const customersElement = document.querySelector(
      '[data-stat="total-customers"]'
    );
    if (customersElement) {
    ...
    if (growthElement) {
      ...
      growthElement.innerHTML = `
            ...
            `;
      growthElement.className = `text-sm mt-2 ${
  ...
  async loadRecentOrders() {
    try {
      const response = await fetch(
        `${this.apiBase}/analytics/recent-orders?limit=5`
      );
      const data = await response.json();
  ...
  updateRecentOrdersTable(orders) {
    ...
    tableBody.innerHTML = orders
      .map((order) => {
        ...
        const paymentStatusClass = this.getPaymentStatusClass(
          order.paymentStatus
        );

        return `
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-sm font-medium text-gray-900">#${order._id.slice(
                      -6
                    )}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">${
                      order.customerName
                    }</td>
                    <td class="px-4 py-3 text-sm text-gray-600">$${order.total.toFixed(
                      2
                    )}</td>
                    <td class="px-4 py-3">
  ...
  async loadTopProducts() {
    try {
      const response = await fetch(
        `${this.apiBase}/analytics/top-products?limit=5`
      );
      const data = await response.json();
            ...
            <div class="flex items-center justify-between py-3 ${
              index < products.length - 1 ? "border-b border-gray-200" : ""
                ...
                <div class="text-right">
                    <p class="text-sm font-medium text-gray-900">$${product.revenue.toFixed(
                      2
                    )}</p>
  ...
  async setupSalesChart() {
    try {
      const response = await fetch(
        `${this.apiBase}/analytics/monthly-sales?months=6`
      );
      const data = await response.json();
  ...
  async setupCategoryChart() {
    try {
      const response = await fetch(
        `${this.apiBase}/analytics/sales-by-category`
      );
      const data = await response.json();
